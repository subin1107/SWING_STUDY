...
rec.clear() #기존 rec 리스트 초기화
nrec.clear() #기존 nrec 리스트 초기화
...
def writerec(): #기록 파일에 저장하는 함수
    ...
    for i in range(0,len(rec),1):
        write=(nrec[i])+str(':')+str(rec[i]) #순위대로 (닉네임:기록) 읽고 저장
        f.write(write)
        f.write("\n")
    f.close()   
        ...
        for i in range(1,11): #총 10번의 기회를 줌
            ...
            if cha<answer and cha>fir: #정답보다 숫자가 작고 가능 범위 안의 숫자면
                ...
                i-=1
                    ...
                    print("GAME OVER! 입력 횟수를 초과하였습니다") #2 오류 수정: i=10일때로 수정. 10번 시도 후에 정답을 맞히지 못했을 떄 출력     
                ...
                if len(rec)==0:  #처음 기록은 무조건 최고 기록이 됨
                    ...
                    rec.append(i) #최고기록을 담아주는 리스트 rec에 기록을 등록(#1: 최고 기록일 때만 기록 등록)
                    nrec.append(maxn) #최고 기록을 가진 닉네임을 담는 리스트 nrec에 기록을 등록
                    break
                elif(rec[0]>i): #최고 기록보다 단축했을시
                    print("최고 기록입니다")
                    print("닉네임을 입력하세요")
                    maxn=(input(">> ")) #최고 기록일 때 이름 입력
                    rec.insert(0, i)   #최고 기록 리스트에서 제일 첫번째 위치에 데이터 삽입(#1: 최고 기록일 때만 기록 등록)
